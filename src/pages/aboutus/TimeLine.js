...
const TimeLine = () => {
  const timeline = [
    ...
    },
    {
      ...
      complete_info:
        "Harry Winston, Inc. purchases a rare 13.22 carat, flawless fancy vivid blue diamond at auction. Named the " + "Winston Blue," + "the magnificent pear-shaped stone has been described as one of the largest of its kind.",
      src: img,
    },
    {
      year: "2017",
          ...
          <div className="yr-ul-container">
            <ul className="year-list-container">
              {timeline.map((timeline_obj) => (
                <li onClick={() => setCurrTimeline(timeline_obj)} className={timeline_obj.year === currTimeline.year && "active"}>{timeline_obj.year}</li>
              ))}
            </ul>
          </div>
